...
const CreateProductForm = ({
  ...
  // Fetch categories and teams on mount
  useEffect(() => {
    if (isOpen) {
      fetchInitialData();
    }
  ...
  const fetchInitialData = async () => {
    try {
      ...
      setTeams([
        { team_id: 1, team_name: "Manchester City", tournament_id: 1 },
        { team_id: 2, team_name: "Arsenal", tournament_id: 1 },
      ]);
    } catch (error) {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      showToast(
        "Chức năng đang phát triển - Sẽ tạo sản phẩm thành công",
        "success"
      );
      onSuccess();
      onClose();
  ...
  const resetForm = () => {
    ...
    setVariants([{ variant_size: "", variant_color: "", variant_stock: 0 }]);
    setImageFile(null);
  };
