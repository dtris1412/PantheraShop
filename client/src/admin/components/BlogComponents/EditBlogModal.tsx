...
const EditBlogModal = ({
  ...
  useEffect(() => {
    if (blog) {
      setFormData({
        ...
        sport_id: blog.sport_id ? String(blog.sport_id) : "",
        team_id: blog.team_id ? String(blog.team_id) : "",
        category_id: blog.category_id ? String(blog.category_id) : "",
        tournament_id: blog.tournament_id ? String(blog.tournament_id) : "",
      });
      setImagePreview(blog.blog_thumbnail || "");
    }
  }, [blog]);
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      await updateBlog(blog.blog_id, updateData);
      showToast("Cập nhật blog thành công!", "success");
      onSuccess();
      onClose();
    } catch (error: any) {
      showToast(error.message || "Có lỗi xảy ra khi cập nhật blog", "error");
    } finally {
      setLoading(false);
    }
  };
